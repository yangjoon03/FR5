...
4. **손을 편 상태(`Open_Palm`)일 때만** 0.15초마다 지금 손 크기(화면 너비 대비 손 박스 너비 비율)와 캘리브레이션해둔 목표 크기를 비교합니다 (`vision_tracker._update_distance_jog`):
   ...
   - 같은 판단 주기에 **수평(좌우)**과 **수직(위아래)**도 각각 독립적으로 처리:
     - 수평(`_update_horizontal_jog`): 손 중심이 화면 중앙 기준 픽셀 데드존(`horizontal_deadzone_px=40`)을 벗어나 오른쪽에 있으면 `jog_start(ref=4, nb=1, dir=1)`, 왼쪽이면 `dir=0`, 중앙 근처면 `jog_stop(ref=5)`.
     - 수직(`_update_vertical_jog`): 마찬가지로 위/아래 픽셀 데드존(`vertical_deadzone_px=40`) 기준으로 `nb=2`(공구 Y축)를 조그.
   - **주먹(`Closed_Fist`)을 쥐거나 다른 모양이면, 혹은 손을 놓치면** 그 즉시 세 축 모두 강제로 멈춤(`ImmStopJOG`) — 이게 사실상의 "실시간 정지 제스처".
...
- 조그는 명령을 안 보낸다고 저절로 멈추지 않으므로(일반 Move와 다른 점), 손을 놓치거나 주먹을 쥐거나 "트래킹 정지"/카메라 닫기를 누르면 **항상 즉시 응답하는 별도 커넥션으로 `ImmStopJOG()`를 강제 호출**해서 확실히 멈춥니다 (축 구분 없는 전역 정지라 세 축을 한 번에 정리함).
- 상단 "정지" 버튼을 누르면 트래킹도 같이 꺼짐.
- 조그 방향(전진/우측/아래 이동이 실제로 의도한 방향이 맞는지 등)은 검증 못 했으므로, 반대로 움직이면 UI의 "거리(전후)"/"수평(좌우)"/"수직(위아래)" 반전 체크박스로 뒤집어서 맞춰야 합니다.

⚠️ **세 축 동시 조그는 실기 검증이 부족합니다.** 처음엔 거리(Z, `nb=3`) 조그 하나만 썼고, 이후 수평(X, `nb=1`)을 추가했고, 이번에 수직(Y, `nb=2`)까지 같은 좌표계(`ref=4`, 툴)에서 독립적으로 추가했습니다. `StartJOG`/`StopJOG`의 `ref`는 좌표계를 가리키지 축을 가리키는 게 아니라서, **한 축을 멈추면 같은 좌표계의 다른 축도 같이 멈춰버릴 가능성**이 있습니다 (예: 수직이 멈춰야 하는 순간 `StopJOG(ref=5)`를 보냈는데, 그게 마침 계속 움직여야 하는 거리·수평 조그까지 같이 끊어버릴 수 있음). 축이 늘어날수록 이 리스크도 커지므로, 처음 테스트할 때 반드시 저속으로, 축들이 서로 멈춰버리거나 간섭하지 않는지 확인하세요.
...
- `StartJOG`의 `max_dis`(단일 조그 최대 이동거리, 500mm)에 도달하면 로봇이 자체적으로 조그를 멈추는데, 소프트웨어는 그걸 모르고 "계속 움직이는 중"으로 착각해서 손이 아주 많이 멀어지면 더 이상 안 따라오는 것처럼 보였음 — 같은 방향이어도 `_JOG_REFRESH_SEC`(1.5초)마다 `StopJOG` 없이 `StartJOG`를 다시 보내 새로고침하도록 고침.
