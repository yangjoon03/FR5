...
| `POST /api/camera/config {invert_pan,invert_tilt,invert_z,invert_horizontal,invert_handedness,max_step_deg,max_step_mm}` | 거리/수평/좌우손 반전 등 설정 (팬/틸트 관련 값은 현재 비활성 기능용으로 남겨둔 것) |
...
4. **손을 편 상태(`Open_Palm`)일 때만** 0.15초마다 지금 손 크기(화면 너비 대비 손 박스 너비 비율)와 캘리브레이션해둔 목표 크기를 비교합니다 (`vision_tracker._update_distance_jog`):
   ...
   - 같은 판단 주기에 **수평(좌우)도 독립적으로** 처리 (`_update_horizontal_jog`): 손 중심이 화면 중앙 기준 픽셀 데드존(`horizontal_deadzone_px=40`)을 벗어나 오른쪽에 있으면 `jog_start(ref=4, nb=1, dir=1)`, 왼쪽이면 `dir=0`, 중앙 근처면 `jog_stop(ref=5)`.
   - **주먹(`Closed_Fist`)을 쥐거나 다른 모양이면, 혹은 손을 놓치면** 그 즉시 두 축 모두 강제로 멈춤(`ImmStopJOG`) — 이게 사실상의 "실시간 정지 제스처".
   - 손목 회전(팬/틸트)은 현재 비활성화 상태 — 거리(전후)+수평(좌우) 평행이동만 동작함.
...
- 조그는 명령을 안 보낸다고 저절로 멈추지 않으므로(일반 Move와 다른 점), 손을 놓치거나 주먹을 쥐거나 "트래킹 정지"/카메라 닫기를 누르면 **항상 즉시 응답하는 별도 커넥션으로 `ImmStopJOG()`를 강제 호출**해서 확실히 멈춥니다 (축 구분 없는 전역 정지라 거리·수평 두 축을 한 번에 정리함).
- 상단 "정지" 버튼을 누르면 트래킹도 같이 꺼짐.
- 조그 방향(전진/우측 이동이 실제로 의도한 방향이 맞는지 등)은 검증 못 했으므로, 반대로 움직이면 UI의 "거리(전후) 반전"/"수평(좌우) 반전" 체크박스로 뒤집어서 맞춰야 합니다.

⚠️ **두 축 동시 조그는 이번이 처음이라 실기 검증이 안 됐습니다.** 지금까지는 거리(Z, `nb=3`) 조그 하나만 썼는데, 이번에 수평(X, `nb=1`) 조그를 같은 좌표계(`ref=4`, 툴)에서 독립적으로 추가했습니다. `StartJOG`/`StopJOG`의 `ref`는 좌표계를 가리키지 축을 가리키는 게 아니라서, **한 축을 멈추면 같은 좌표계의 다른 축도 같이 멈춰버릴 가능성**이 있습니다 (즉 수평이 멈춰야 하는 순간 `StopJOG(ref=5)`를 보냈는데, 그게 마침 계속 움직여야 하는 거리 조그까지 같이 끊어버릴 수 있음). 처음 테스트할 때 저속으로, 두 축이 서로 멈춰버리거나 간섭하지 않는지 꼭 확인하세요.
